...
@Component @RepositoryEventHandler(Journal.class) public class JournalEventHandler {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired private UserRepository userRepository;

	@Value("${global.anniversary}") private String anniversary;

	@HandleBeforeCreate public void handleJournalCreate(Journal journal) {
		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
		LocalDate publishDate = journal.getPublishDate().toInstant().atZone(ZoneId.systemDefault())
				.toLocalDate();
		LocalDate anniversaryDate = LocalDate
				.parse(this.anniversary, DateTimeFormatter.ofPattern("yyyy-MM-dd"));
